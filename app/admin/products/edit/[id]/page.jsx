...
export default function EditProductPage() {
    ...
    const fetchProduct = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:5000/api/products/admin/${productId}`);
            ...
            if (product.thumbImg) {
                setExistingThumb(product.thumbImg);
                setThumbPreview(`http://localhost:5000${product.thumbImg}`);
            }

            if (product.galleryImg && product.galleryImg.length > 0) {
                setExistingGallery(product.galleryImg);
                setGalleryPreviews(product.galleryImg.map(img => `http://localhost:5000${img}`));
            }
        } catch (error) {
            toast.error("Failed to fetch product details");
            console.error("Product fetch error:", error);
            setTimeout(() => router.push("/admin/list-products"), 2000);
        } finally {
    ...
    // Handle short description CKEditor change
    const handleShortDescChange = (event, editor) => {
        const data = editor.getData();
    ...
    const validateField = (name, value) => {
        ...
        switch (name) {
            ...
            case 'category':
                if (value.length === 0) {
                    fieldErrors.category = "Select at least one category";
                } else {
    ...
    const handleCategoryChange = (e) => {
        ...
        validateField('category', selected);
    };
    ...
    const removeGalleryImage = (index) => {
        ...
        if (isExisting) {
            // Track existing images to remove on server
            ...
            setSelectedFiles(prev => ({
                ...prev,
                galleryImg: prev.galleryImg.filter((_, i) => i !== newIndex),
            }));
            setGalleryPreviews(prev => prev.filter((_, i) => i !== index));
        }
    };
        ...
        try {
            ...
            toast.error(
                <div className="flex items-center gap-2">
                    ...
                    <span>{error.response?.data?.message || "Failed to update product"}</span>
                </div>,
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            onBlur={handleBlur}
                                            required
                                            className={`w-full px-4 py-3 pl-11 border rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all ${
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            onBlur={handleBlur}
                                            required
                                            className={`w-full px-4 py-3 pl-11 border rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all bg-gray-50 ${
                                    ...
                                    {categoriesLoading ? (
                                        ...
                                        <>
                                            <select
                                                ...
                                                className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all min-h-[160px] ${
                            ...
                            <div className="px-6 py-5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-200">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600">Update product photos</p>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    {!thumbPreview ? (
                                        <div className="relative">
                                            ...
                                            <label
                                                ...
                                                className={`cursor-pointer flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-xl transition-all ${
                                                    errors.thumbImg
                                                        ? "border-red-300 bg-red-50"
